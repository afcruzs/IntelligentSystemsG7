...
public class SecondAgentGrupo7 implements AgentProgram, Grupo7If {
	...
	protected final int REMAIN_MOTIONLESS = 70;
	protected final int REMAIN_MOTIONLESS_SEARCHING_PROB = 70;
	protected final int RESET_ON_LOOP_PROB = 80;
	...
	public void save(boolean updateLast) {
		...
		if (updateLast){
			...
			addToHistory( lastCriticalCoordinate );
			lastCriticalCoordinate = current.clone();
		}
	}
...
//		posibleCoordinates.poll();
	...
	private Coordinate goToNode(Coordinate target) {
	...
	void avoidLoopByTimesBlocked(boolean FA){
		...
		int prom = (REMAIN_MOTIONLESS+REMAIN_MOTIONLESS_SEARCHING_PROB)/2;
		double factor = 1.0 + prom/100; 
		if(FA && timesBlocked >= factor*MAX_BLOCKED_TIMES){
			if( r.nextInt(100) < RESET_ON_LOOP_PROB ){
				//JOptionPane.showMessageDialog(null, "Machete extremo");
				clearMemory();
			}
		}
		...
		/*if(FA && timesBlocked >= factor*MAX_BLOCKED_TIMES){
			...
			if( map.size() == 1 ){
				...
				Coordinate randomNode = null;
				do{
					randomNode = map.randomNode();
					// System.out.println( randomNode );
				}while( randomNode.equals(lastCriticalCoordinate) );
				
				if( !randomNode.equals(lastCriticalCoordinate) ){
					lastCriticalCoordinate = randomNode;
					cmd.clear();
					goToNode( lastCriticalCoordinate );
				}else{
					JOptionPane.showMessageDialog(null, "Machete extremo");
					clearMemory();
				}
	...
	void addToHistory(Coordinate c){
		
		if( c == null ) return;
		
		if( lastCriticalHistory.size() == 0 )
			lastCriticalHistory.add(c);
		else if( !lastCriticalHistory.peek().equals(c) )
					lastCriticalHistory.add(c);
	...
	public void actWhenAgentInterrupted(boolean FW, boolean RW, boolean BW, boolean LW, boolean FA, boolean RA, 
		...
		if( currentOperation == SEARCHING ){
			...
			if( r.nextInt(100) < REMAIN_MOTIONLESS_SEARCHING_PROB ){
		...
		if( r.nextInt(100) < REMAIN_MOTIONLESS || 
				currentOperation == GO_BACK ) {
			// System.out.println( "NO_OP" );
			cmd.add(0, language.getAction(NO_OP));
			return;
