...
public class IterativeAStar extends RubikSearch{

	private RubikHeuristic heuristic;
	private double limitF;
	
	public IterativeAStar(RubikHeuristic heuristic, double limitF) {
	...
	public List<RubikAction> doSearch(RubikCube cube) {
		...
		while( !q.isEmpty() ){
			current = q.poll();
			if( current.state.depth > limitF ) continue;
			expandedNodes++;
